...
DialogConfigPack::DialogConfigPack(QWidget *parent)
    : QDialog(parent), ui(new Ui::DialogConfigPack) {
    ui->setupUi(this);
    ...
    m_pSet = new QSettings(QCoreApplication::applicationDirPath() +
                               "/TCLBinConfig.ini",
                           QSettings::IniFormat);
    ...
    m_pAModel->setHorizontalHeaderLabels(
        {"输出", "D", "左移", "Bytes", "Hex", "备注", "文件", "操作(点击)"});
    ...
    ui->tableViewRam->setStyleSheet(
        "QTableView::indicator { margin-left: 6px; }");
    ...
    connect(ui->tableViewRam, &QTableView::clicked, this,
            [=](const QModelIndex &index) {
                m_nSelectRam = index.row();

        if (index.column() == 7) {
                    QString strFile = QFileDialog::getOpenFileName(
                this, "选择 RAM 配置文件", nullptr,
    ...
    connect(
        ui->tableViewRam->selectionModel(),
    ...
    connect(ui->lineEditXlsxFile, &QLineEdit::textChanged, this,
            [=](const QString &arg1) {
                m_bLoading = true;
        ...
        ui->comboBoxSheets1->setCurrentIndex(-1);
                QTimer::singleShot(
            100, this, [=] { ui->comboBoxSheets1->setCurrentIndex(1); });
    });

    ui->lineEditXlsxFile->setText(m_pSet->value("lastXlsxFile").toString());
    ui->lineEditSavePath->setText(
        m_pSet
            ->value("binFilePath",
                    QCoreApplication::applicationDirPath() + "/binfiles")
            .toString());
    ...
    connect(ui->comboBoxSheets1, &QComboBox::currentTextChanged, this,
            [=](const QString &arg1) {
                if (!m_bLoading)
            showSheet(arg1);
    });
    ...
    connect(ui->pushButtonGetSavePath, &QPushButton::clicked, this, [=] {
        QString strPath = QFileDialog::getExistingDirectory(
            this, "选择BIN文件输出目录", ui->lineEditSavePath->text().trimmed());
        if (strPath.isEmpty())
            return;
    ...
    connect(ui->pushButtonGotoSavePath, &QPushButton::clicked, this, [=] {
        QDesktopServices::openUrl(
            QUrl::fromLocalFile(ui->lineEditSavePath->text().trimmed()));
    });

    connect(m_pRModel, &QStandardItemModel::itemChanged, this,
            [=](QStandardItem *item) {
                if (m_bLoading)
            return;
                int nRow = item->row();
                ...
                continue;
                    ...
                    if (nCol == 0) {
                QString strAddr =
                            QString::asprintf("%04X", strText.toInt(nullptr, 16));
                DI.strAddr = strAddr;
                ...
                break;
                    ...
                    QString strVal =
                        QString::asprintf("%02X", strText.toInt(nullptr, 16));
    ...
    ui->tableViewReg->setStyleSheet(
        "QTableView::indicator { margin-left: 6px; }");
    connect(ui->tableViewReg, &QTableView::clicked, this,
            [=](const QModelIndex &index) { Q_UNUSED(index) });

    connect(ui->tableViewReg->horizontalHeader(), &QHeaderView::sectionClicked,
            this, [=](int index) {
        if (index >= 3)
    ...
    connect(ui->pushButtonBuildCfg, &QPushButton::clicked, this, [=] {
        ...
        for (int i = 0; i < nCount; i++) {
            ...
            while (!in.atEnd())
                Lines.push_back(in.readLine().trimmed());
            RamFile.close();
        ...
        D.mkdir(strPath);
        QString strFile =
            QString("%1/KLNS_%2_%3.txt").arg(strPath, strModel, strTag);
        ...
        if (textFile.open(QIODevice::WriteOnly)) {
            ...
            QString strBinFile =
                QString("%1/KLNS_%2_%3.bin").arg(strPath, strModel, strTag);
...
DialogConfigPack::~DialogConfigPack() { delete ui; }

bool DialogConfigPack::eventFilter(QObject *watched, QEvent *event) {
    QApplication::restoreOverrideCursor();

    if (watched == ui->tableViewRam) {
        if (event->type() == QEvent::Paint || event->type() == QEvent::ToolTip) {
...
void DialogConfigPack::saveLoadRamCfg(bool bToSave) {
    if (bToSave) {
        ...
        for (int i = 0; i < nCount; i++) {
            ...
            QString strLine = QString::asprintf(
                "%d,%d,%d,%d,%d,%s,%s", item0->checkState() == Qt::Checked,
                item1->text().toInt(), item2->text().toInt(), item3->text().toInt(),
        ...
        for (int i = 0; i < nCount; i++) {
            ...
            QStringList Values =
                m_pSet->value("RamSetting/RamItem" + QString::number(i))
                                     .toString()
                                     .split(',');
...
void DialogConfigPack::dropEvent(QDropEvent *event) {
    ...
    if (mimeData->hasUrls()) {
        ...
        rc1 = QRect(ui->groupBox1->mapToParent(rc1.topLeft()),
                    ui->groupBox1->mapToParent(rc1.bottomRight()));
        rc2 = QRect(ui->groupBox2->mapToParent(rc2.topLeft()),
                    ui->groupBox2->mapToParent(rc2.bottomRight()));
...
void DialogConfigPack::loadFile(const QString &strFile) {
    ...
    {
        m_readCols = QStringList{"C", "G", "I"};
        QStringList sheets = xlsx.sheetNames();

        QFile file("D:\\akkodevice.txt");
        ...
        foreach (const QString &strSheet, sheets) {
            if (!xlsx.selectSheet(strSheet))
                continue;
            qDebug().noquote() << "//" << strSheet;

            out << "//" << strSheet << "\n";
            for (int i = 2; i < 2000; i++) {
                QString strId, strName, strVid;
                for (int col = 0; col < m_readCols.count(); col++) {
                    QString strCell = QString::asprintf(
                        "%s%d", m_readCols[col].toStdString().c_str(), i);
                    strCell = QString("%1%2").arg(m_readCols[col]).arg(i);
                ...
                if(strVid.isEmpty() && strId.isEmpty() && strName.isEmpty())
                    break;
                if (strId.trimmed().isEmpty())
                    break;
                // if(strVid.trimmed().length() < 9) break;
                ...
                QString strLine = QString("{") + strId + QString(",") + strVid +
                                  QString(",0,\"") + strName.trimmed() + "\"},";
                // qDebug().noquote() << "{" << strId<< "," << strVid <<  << strName <<
                // "\"}";
                qDebug().noquote() << strLine;

                out<< strLine << "\n";

                QThread::msleep(5);
                //strLine.split(',').at(0);
                //strOunt.append(strLine);
            }
        }
        //qDebug().noquote() << strOunt;
        return;
    }

    if (!xlsx.selectSheet("REG.ld_ip")) {
        QMessageBox::warning(this, "提示",
                             "此XLSX文档不符合约定格式，无法继续读取数据！");
        return;
    }
...
void DialogConfigPack::loadSheet(Document &xlsx, const QString &strSheet) {
    ...
    for (int i = 3; i < 2000; i++) {
        ...
        for (int col = 0; col < m_readCols.count(); col++) {
            QString strCell =
                QString::asprintf("%s%d", m_readCols[col].toStdString().c_str(), i);
            QString strValue = xlsx.read(strCell).toString().trimmed();
            if (strValue.isEmpty() && col == 0) {
                bEnding = true;
                break;
